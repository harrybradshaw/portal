...
if(! empty( $id ) ) {
  $res = dbi_execute('SELECT DMGuser FROM webcal_entry WHERE cal_id = ?', array($id));
  if($res){
    $row = dbi_fetch_row( $res );
    $OldDMGuser = $row[0];
  }
...
if( empty( $error ) ) {
  ...
  //hlb54 23/07/2019
  if(empty( $OldDMGuser )) {
    dbi_execute('UPDATE webcal_entry SET DMGuser = ? WHERE cal_id = ?', array($_COOKIE['DMGuser'], $id));
  }else{
    dbi_execute('UPDATE webcal_entry SET DMGuser = ? WHERE cal_id = ?', array($OldDMGuser, $id));
  }
