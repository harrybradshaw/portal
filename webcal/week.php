...
//hlb54: This adds on the custom topbar, if the user is authenticated. 
...
if(isset($_COOKIE['DMGuser'])){
  ...
  include '../db_connect.php';
	...
	$login = '__public__';
  include '../topbar_noverif.php';
  header("location:../unt.php");
}
echo '<div style="padding-top:50px;padding-left:30px;padding-right:30px">';
if(substr($login,0,3) == 'rm_'){
  //echo 'Room LOGIN';
  $roomFlag = TRUE;
  echo '<form name="makelist" method="post" action="login.php"> Change room: ';
  echo '<select name="rooms">';
  //echo "<option value='rm_1p015'>Left Measuring Lab</option>";
  //echo "<option value='rm_1p016'>CryoFree Lab</option>";
  //echo "<option value='rm_1p019'>Sputter Room</option>";
  //echo "<option value='rm_1p021'>Clean & Dirty Prep</option>";
  
  $sql = mysqli_query($conn_equipt, "SELECT * FROM Rooms ORDER BY RoomName");
  ...
  echo '</form>';
  $sql = "SELECT * FROM Rooms WHERE RoomID = '".$login."'";
  $res = $conn_equipt->query($sql);
  foreach ($res as $value){
    $MaxO = $value['MaxO'];
  }
  echo '<h2><center>Maximum Room Occupancy: ' . $MaxO. '</center></h2>';
  
}else{
  $sql = "SELECT * FROM EQ_LIST WHERE abbrev = '".$login."'";
  $res = $conn_equipt->query($sql);
  foreach ($res as $value){
    $room = $value['Room'];
  }
...
print_header(
  ...
  generate_refresh_meta(), '', false, false, false, false,false,false);
...
if($login != '__public__' && $roomFlag!=TRUE){
  $entryurl = 'edit_entry.php?date='.date("Ymd").'&defusers=&hour='.date("G").'&minute=0';
  //echo '<a href='.$entryurl.'><h3>Add booking</h3></a>';
  echo '<form method="post" action="'.$entryurl.'">';
  echo '<p><input type="submit" value="Add booking"> </p>';
  echo '</form>';
}
echo '<br>';
echo '(Additional Calendars For: ';
$sql = "SELECT cal_layeruser FROM webcal_user_layers WHERE cal_login = '".$login."'";
    $res = $conn_webcam->query($sql);
    foreach($res as $value){
      $sql_a = "SELECT fullname FROM EQ_LIST WHERE abbrev = '".$value['cal_layeruser']."'";
      $res_a = $conn_equipt->query($sql_a);
      foreach($res_a as $value_a){
        echo $value_a['fullname'].', ';
      }
